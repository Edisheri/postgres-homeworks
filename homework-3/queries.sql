...
JOIN orders USING (customer_id) JOIN employees USING (employee_id) JOIN shippers s on s.shipper_id = orders.ship_via
...
INNER JOIN suppliers USING(supplier_id) INNER JOIN categories USING(category_id)
...
SELECT company_name FROM customers LEFT JOIN orders o on customers.customer_id = o.customer_id WHERE o.customer_id IS NULL
